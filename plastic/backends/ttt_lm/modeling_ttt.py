...
# ---------------------------------------------------------------------------------------------
# plastic instrumentation (added; upstream has none). Thread-local so concurrent sessions on one
# model never race:
#   _tls.eta_scale  multiplies the learned per-token inner learning rate. 1.0 = native; 0.0 = a
#                   genuine no-write (fast weights and their pending gradients stay exactly equal;
#                   conv states and the position still advance). TTT has no decay, so freeze and
#                   beta_scale=0 coincide on this architecture.
#   _tls.collect    when a list, every mini-batch step appends per-token signals per layer:
#                   the inner reconstruction loss (surprise), the effective step size (eta) and the
#                   exact per-token fast-weight update norm (each token's update is rank one, so
#                   ||eta_t x_t g_t^T|| = eta_t ||x_t|| ||g_t||; biases add eta_t ||g_t||).
# ---------------------------------------------------------------------------------------------
...
def _eta_scale() -> float:
    return float(getattr(_tls, "eta_scale", 1.0))


def _collector():
    return getattr(_tls, "collect", None)
...
def _cached_causal_conv(conv: nn.Conv1d, x: torch.Tensor, states: dict, layer_idx: int) -> torch.Tensor:
    """plastic: a cached depthwise causal conv for ANY number of new tokens (upstream's cached path handled
    one token at a time). ``states[layer_idx]`` holds the last K inputs (most recent last), as upstream
    stores them; for L == 1 this reproduces upstream's roll-and-sum exactly."""
    K = conv.weight.shape[-1]
    L = x.shape[-1]
    inp = torch.cat([states[layer_idx].to(x.dtype), x], dim=-1)  # [B, C, K+L]
    out = conv(inp)[..., K:K + L]
    states[layer_idx].copy_(inp[..., -K:])
    return out
        ...
        if causal_conv1d_fn is None:
            if cache_params is not None:
                if cache_params.seqlen_offset > 0:
                    hidden_states = _cached_causal_conv(self.conv, hidden_states, cache_params.conv_states_dic["pre_conv"], self.layer_idx)
                else:
...
class TTTCache:
    ...
    def update(self, py_tree, layer_idx, seq_len):
        if getattr(self, "no_update", False):  # plastic: frozen probe on grad leaves; states must not be copied into
            return
...
class TTTBase(nn.Module):
    ...
    def get_qkv_projections(self, hidden_states, cache_params: Optional[TTTCache] = None):
        if self.share_qk:
            ...
            if causal_conv1d_fn is None:
                if cache_params is not None:
                    if cache_params.seqlen_offset > 0:
                        XQ = _cached_causal_conv(self.conv_q, xq, cache_params.conv_states_dic["ttt_conv_q"], self.layer_idx)
                        XK = _cached_causal_conv(self.conv_k, xq, cache_params.conv_states_dic["ttt_conv_k"], self.layer_idx)
                    else:
    ...
    def get_ttt_inputs(self, inputs, mini_batch_size, cache_params):
        ...
        ttt_lr_eta = ttt_lr_eta * _eta_scale()  # plastic: freeze / beta_scale control
...
class TTTLinear(TTTBase):
    ...
    def ttt(
        ...
        def compute_mini_batch(params_dict, inputs):
            ...
            col = _collector()
            if col is not None:  # plastic signals, per token: [B, nh, K]
                with torch.no_grad():
                    err = (ln_fwd(Z1, ln_weight, ln_bias) - reconstruction_target).norm(dim=-1)
                    step = (token_eta_mini_batch[..., 0] * ttt_lr_eta_mini_batch[..., 0, :])
                    last_tok_eta = torch.clamp_min(self.token_idx + self.learnable_token_idx, 0.0)[-1]
                    wn = last_tok_eta * ttt_lr_eta_mini_batch[..., 0, :] * torch.sqrt((X1.norm(dim=-1) ** 2 + 1.0) * grad_l_wrt_Z1.norm(dim=-1) ** 2)
                    col.append({"layer": self.layer_idx, "err": err, "eta": step, "write_norm": wn})
...
class TTTMLP(TTTBase):
    ...
    def ttt(
        ...
        def compute_mini_batch(params_dict, inputs):
            ...
            if col is not None:  # plastic signals, per token: [B, nh, K]
                with torch.no_grad():
                    err = (ln_fwd(Z2, ln_weight, ln_bias) - reconstruction_target).norm(dim=-1)
                    # beta: the step applied to token t's own read, token_eta[t] * ttt_lr_eta[t]
                    step = (token_eta_mini_batch[..., 0] * ttt_lr_eta_mini_batch[..., 0, :])
                    # write_norm: the exact norm of token t's rank-one contribution to the COMMITTED update, which
                    # the reference scales by the LAST position's token_eta once the mini-batch completes
                    last_tok_eta = torch.clamp_min(self.token_idx + self.learnable_token_idx, 0.0)[-1]
                    wn = last_tok_eta * ttt_lr_eta_mini_batch[..., 0, :] * torch.sqrt(
                        (X1.norm(dim=-1) ** 2 + 1.0) * grad_l_wrt_Z1.norm(dim=-1) ** 2
                        + (X2.norm(dim=-1) ** 2 + 1.0) * grad_l_wrt_Z2.norm(dim=-1) ** 2
                    )
                    col.append({"layer": self.layer_idx, "err": err, "eta": step, "write_norm": wn})
