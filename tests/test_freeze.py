...
@pytest.mark.parametrize("n_frozen", [1, 6, 7, 9, 20])
def test_chunk_rule_resume_after_freeze_keeps_retention(device, n_frozen, mode):
    """Frozen tokens must not make the next applied chunk decay by a truncated average.
    Resume exactly to the next boundary so the update is exercised in every case."""
    cfg = ModelConfig(d_model=32, n_heads=2, chunk=8, rule="chunk")
    mem = FastWeightMemory(cfg).to(device)
    n_active = 8 - (n_frozen % 8)
    u = torch.randn(1, n_frozen + n_active, 32, device=device)
    st = SessionState.zeros(cfg, batch=1, device=device).layers[0]
    st.S += torch.randn_like(st.S)
    _, S1, M1, c1, _ = mem(u[:, :n_frozen], st.S, st.M, mode=mode, freeze=True, chunk0=st.chunk)
    assert torch.equal(S1, st.S) and c1.count == n_frozen % 8
    # resume with writes disabled: the boundary applies only the mean retention over the chunk
    _, S2, _, c2, sig = mem(u[:, n_frozen:], S1, M1, mode=mode, beta_scale=0.0, chunk0=c1)
    assert c2.count == 0
    alphas = sig.alpha.mean(dim=-1)  # only the active tokens carry alpha < 1
    expected = ((8 - n_active) * 1.0 + n_active * alphas) / 8
    assert torch.allclose(S2, expected[..., None, None] * S1, atol=1e-5)
    assert float((S2.norm() / S1.norm())) > 0.9
