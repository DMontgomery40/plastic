...
def test_run_modes_uses_a_fresh_model_per_mode_and_tables_render(tmp_path):
    ...
    reports = run_modes(_factory, modes=("frozen", "continued", "retrieval"), spec=spec, seed=0, lr=1e-3, steps=1, device=torch.device("cpu"))
    assert reports["retrieval"]["compute"]["parameters"] == 0 and reports["retrieval"]["learner"]["k"] == 8
    ...
    assert table.count("\n") == 4 and "| frozen |" in table and "| continued |" in table and "| retrieval |" in table
    ...
    assert sorted(os.listdir(tmp_path)) == ["README.md", "continued.json", "frozen.json", "manifest.json", "retrieval.json"]
