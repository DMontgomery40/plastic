...
| `surprise_vs_entropy_step250` | final chat checkpoint, step 250 | 29e0f8558d157cbf | ccfa446 + the corrected-digest script edit committed next (recorded `ccfa446+dirty`; the JSON has no `device` field because of a comment bug fixed in the same commit; the run was on MPS per the pipeline's own execution record) | repeat of step 4 on the final checkpoint, same protocol, assistant tokens: entropy Spearman 0.86 / AUC 0.85; surprise 0.16 / 0.57; increment point estimates +0.000015 R² and +0.00021 AUC (bootstrap means +0.00011 / +0.00021; the R² interval stays above zero, the AUC interval crosses it); loop AUC entropy 0.42 / 0.39, surprise 0.50 / 0.38 at T 0.3 / 0.7 (score direction, not a joint test). For these runs: tiny increment, no surprise-driven sampler |
