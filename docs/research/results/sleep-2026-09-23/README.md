...
| `surprise_vs_entropy_step250` | final chat checkpoint, step 250 | 29e0f8558d157cbf | ccfa446 + the corrected-digest script edit committed next (recorded `ccfa446+dirty`; the JSON's `device` field is null because of a comment bug fixed in the same commit; the run was on MPS per its log) | repeat of step 4 on the final checkpoint, same protocol: entropy Spearman 0.86 / AUC 0.85; surprise 0.16 / 0.57; increments +0.0001 R² and +0.0002 AUC (bootstrap intervals include zero); loop AUC entropy 0.42 / 0.39, surprise 0.50 / 0.38 at T 0.3 / 0.7. Question closed: no surprise-driven sampler |
